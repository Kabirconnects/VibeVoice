...
>
> 🔗 **Clone this repo to use it:** `https://github.com/Kabirconnects/VibeVoice`
...
```bash
git clone https://github.com/Kabirconnects/VibeVoice.git
cd VibeVoice/
...
> [!TIP]
> **One install command covers both modes** — local (model download) *and* online API. No extra packages needed.

> [!NOTE]
> **If you run without a `.env` file**, VibeVoice defaults to **local mode** and automatically downloads the **`vibevoice/VibeVoice-1.5B`** model from HuggingFace (~3GB), requiring a GPU. To skip the download, create a `.env` and enable API mode (see below).

---

## Configuration (`.env`)

VibeVoice reads your settings from a `.env` file in the project root, which controls everything:
...
### 1. API mode — ✅ easiest, no GPU, no model download

TTS is generated by an **online provider**. Just pick a provider, then run the app.
...
VIBEVOICE_MODE=api
API_PROVIDER=gemini
API_KEY=your-google-ai-studio-key      # get one free: https://aistudio.google.com/apikey
API_MODEL=gemini-2.5-flash-preview-tts
API_VOICE=Kore
API_VOICES=Kore,Puck,Charon,Aoede       # one voice per speaker (multi-speaker)
```
...
VIBEVOICE_MODE=api
API_PROVIDER=openai
API_KEY=sk-...                          # from https://platform.openai.com/api-keys
API_BASE_URL=https://api.openai.com/v1
API_MODEL=tts-1
API_VOICE=alloy
API_VOICES=alloy,echo,fable,onyx        # one voice per speaker
...
API_KEY=your-elevenlabs-key             # from https://elevenlabs.io
API_MODEL=21m00Tcm4TlvDq8ikWAM          # voice ID
API_VOICE=eleven_multilingual_v2
API_VOICES=VOICE_ID_1,VOICE_ID_2        # one voice ID per speaker
API_AUDIO_FORMAT=pcm_24000
```
...
VIBEVOICE_MODE=api
API_PROVIDER=google
API_KEY=AIza...                         # Google Cloud API key (Cloud TTS API enabled)
API_MODEL=en-US-Neural2-D
API_VOICE=1.0                           # speaking rate, 1.0 = normal
API_VOICES=en-US-Neural2-D,en-US-Neural2-F
API_AUDIO_FORMAT=LINEAR16
```

> `API_VOICES` = **one voice per speaker** (comma-separated, in order). Omit it and all speakers share `API_VOICE`. See [.env.example](./.env.example) for every option and voice lists.

### 2. Local mode — 🔧 GPU required, model auto-downloads

```env
VIBEVOICE_MODE=local
...
Opens at `http://127.0.0.1:7860` — paste a script, pick voices, generate.

### CLI — from a script file

```bash
# API or local mode — same command, config decides
python demo/inference_from_file.py --txt_path demo/text_examples/1p_abs.txt    # 1 speaker
python demo/inference_from_file.py --txt_path demo/text_examples/2p_coding.txt # 2 speakers
```

Output is saved to `./outputs/`.

> [!NOTE]
> **Multi-speaker scripts** (e.g. `Speaker 1:` / `Speaker 2:`) are handled automatically. In **API mode**, each speaker gets its own voice from `API_VOICES` and segments are stitched together. In **local mode**, pass speaker names: `--speaker_names Andrew Ava`.

## What happens without a `.env`?
